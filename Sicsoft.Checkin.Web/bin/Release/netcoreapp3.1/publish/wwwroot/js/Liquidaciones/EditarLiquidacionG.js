                    ...
                    success: function (json) {
                        ...
                        if (json == "true") {
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    window.location.href = window.location.href.split("/EditarG")[0];
                                }
                            })
...
function mask2() {
    try {
        var selectedType = document.getElementById('selectTP').value;
        var inputDPI = document.getElementById('CodProveedor');

        if (selectedType === '01') {
            // Máscara para DPI Natural (ejemplo: 0000 00000 0000)
            $(inputDPI).inputmask({ "mask": "9999 99999 9999", autoUnmask: true });
        } else if (selectedType === '02') {
            // Máscara para DPI Jurídico (ejemplo: 0000000-0)
            $(inputDPI).inputmask({ "mask": "9999999-9", autoUnmask: true });
        } else if (selectedType === '03') {
            // Máscara para Pasaporte Extranjero (ejemplo: P-0000000)
            $(inputDPI).inputmask({ "mask": "P-9999999", autoUnmask: true });
        }
    } catch (e) {
        Swal.fire({
            icon: 'error',
